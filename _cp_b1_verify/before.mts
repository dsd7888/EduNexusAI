...
async function main() {
...

  {
    ...
    await page.screenshot({ path: `${OUT}/before-desktop-long-scroll.png`, fullPage: true });
    const height = await page.evaluate(() => document.body.scrollHeight);
    console.log("[before] desktop full-page scrollHeight:", height, "px");
    await ctx.close();
  }

  {
    ...
    await page.waitForSelector("text=Placement Prep", { timeout: 15000 });
    await page.waitForTimeout(400);
    await page.screenshot({ path: `${OUT}/before-mobile-long-scroll.png`, fullPage: true });
    const height = await page.evaluate(() => document.body.scrollHeight);
    console.log("[before] mobile full-page scrollHeight:", height, "px");
    await ctx.close();
  }
