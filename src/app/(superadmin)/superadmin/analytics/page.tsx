...
export default function SuperadminAnalyticsPage() {
  ...
  const fetchSummary = async () => {
    try {
      ...
      if (!res.ok) {
        console.error("[analytics/summary] API error:", json.error);
        setSummary(null);
        return;
      }
      setSummary(json);
  ...
  const cacheHitRate = useMemo(() => {
    if (!data) return "—";
    const { total_entries, total_hits } = data.cacheStats;
    const denom = total_hits + total_entries;
    if (!denom) return "—";
    return `${Math.round((total_hits / denom) * 100)}%`;
  }, [data]);
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm">
              Overview of student engagement, quizzes, and generated content
              across all subjects.
            </p>
          </div>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            ...
            <Presentation className="size-5 text-purple-500" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-semibold">
      ...
      {/* Rest of the analytics UI (same as faculty) */}
      {isLoading ? (
        <PageSkeleton />
      ) : !data ? (
        <p className="text-muted-foreground text-sm">
        ...
        <>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">
                  % of questions answered from cache (free)
                </p>
              </CardContent>
            </Card>
          </div>

          {/* CHARTS ROW (same as faculty) */}
          ...
          {/* QUIZ PERFORMANCE TABLE (same as faculty) */}
          <Card>
